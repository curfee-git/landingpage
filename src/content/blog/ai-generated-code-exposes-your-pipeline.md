...
This approach worked because the scope was limited. A lead engineer could keep the system landscape in mind, remember the naming conventions, verify that a new service adhered to the layer rules, and intervene if anything diverged from them. The pipeline didn't need to enforce the structure, as this was already handled by the engineers upstream.
...
This change is not abstract. A reviewer whose value lay in identifying structural inconsistencies has less to do when the pipeline detects them first. A staff engineer whose unofficial role was to preserve the architectural memory no longer has to keep this in mind and no longer receives recognition for it. The informal power that stemmed from knowledge of the unwritten rules fades away when these rules are codified as executable checks.
